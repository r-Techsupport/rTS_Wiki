---
title: Recovering files with a Linux USB
description: A guide to using a Linux live USB to access and recover files from a disk, including mounting the disk and copying files to another location.
sidebar:
    hidden: false
has_children: false
parent: Data Recovery
grand_parent: Disks
pagefind: true
last_modified_date: 2024-02-23
---

import UseLLCD from '../../../../_includes/embeds/use-linux-live-session.md';

<UseLLCD />

Open the File Manager from the applications menu, you should see your disk in the pane on the left. In the below image, it is the "11GB Volume"

![image1](../../../../assets/data-recovery/dr0.webp)

Double click the desired disk and it should open in the file manager.

![image2](../../../../assets/data-recovery/dr1.webp)

Once opened, you can drag and drop files to another disk that you mount into the system just like in Windows.