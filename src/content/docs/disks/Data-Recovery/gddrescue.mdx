---
title: GNU ddrescue
description: GNU ddrescue is a command-line tool that copies data from a bad drive to an image or another drive while intelligently skipping and retrying bad sectors, making it much safer and more efficient than using plain dd on failing disks.
sidebar:
    hidden: false
has_children: false
...
> [!NOTE] Information
>
> GNU ddrescue is one of the preinstalled packages for our [r/Techsupport Rescue Media](/live-sessions/linux-live-session). If you are using this live image, you can skip the installation section.

> [!TIP] How to install GNU ddrescue
>
> Refer below for installation instructions if you are not using our live image or want to install it on your own system.
>
> <details markdown='1'>
>   <summary>Installing GNU ddrescue</summary>
>
...
> [!IMPORTANT] Important
>
> The command may point to a `ddrescue` binary even though the application is called GNU ddrescue. For the rest of the guide, we will refer to the package as `ddrescue`.

## Basic usage

### What GNU ddrescue does
...
Please refer to this guide on how to identify your drives in Linux: [How to identify failing drives in Linux](/disks/identify-drives-linux). 

Always double-check which drive is source and which is destination before running GNU ddrescue, because mixing them up can destroy your only copy of the data.

### Running GNU ddrescue: Clone to image file
...
- `-n` tells GNU ddrescue to avoid scraping and retries for now; it copies all readable blocks as quickly as possible, skipping areas that cause errors.
...
- `-r3` tells GNU ddrescue to retry bad sectors up to 3 times; you can adjust the number, but extremely high retry counts can be risky and very slow.
...
### Running GNU ddrescue: Cloning directly disk-to-disk
...
The logfile (often called mapfile) is a critical part of GNU ddrescue's operation. It tracks the status of each block of data.

- You must specify the same logfile every time you work on a given source-destination pair.
- The logfile records which blocks are finished, failed, or pending; GNU ddrescue uses this to avoid re-reading good areas and to focus on bad regions.
- If the system crashes or you stop GNU ddrescue, you can simply rerun the last command and it resumes from where it left off using the logfile.
...
Basically the pattern to follow is "GNU ddrescue -> image file -> run other recovery tools on that image".